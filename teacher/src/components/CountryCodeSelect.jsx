  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center gap-1.5 h-[42px] px-3 border rounded-md focus:ring-2 focus:outline-none whitespace-nowrap"
        style={{
      ...
      {/* Dropdown */}
      {showDropdown && (
        <div className="absolute z-50 w-72 mt-1 bg-white border rounded-md shadow-lg max-h-64 overflow-hidden"
            ...
            {filteredCountries.length === 0 ? (
              ...
              filteredCountries.map((c, i) => (
                <button
                  ...
                  className="w-full px-3 py-2 text-left hover:bg-gray-50 flex items-center gap-2.5 transition-colors"
                >
                  <span className="text-xl leading-none flex-shrink-0">{c.flag}</span>
                  <span className="text-sm flex-1 truncate text-gray-700">{c.country}</span>
                  <span className="text-sm text-gray-400 flex-shrink-0 font-medium">{c.code}</span>
                </button>
              ))
            )}
          </div>
