...
export default function TestsPage() {
  ...
  const handleStartTest = (test) => {
    navigate(`/tests/${test.testId}?submissionId=${test.submissionId}`);
  };

  const handleRetakeTest = (test) => {
    navigate(`/tests/${test.testId}?submissionId=${test.submissionId}&retake=true`);
  };
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {(["NOTSUBMITTED", "SUBMITTED", "GRADED"]).map((status) => {
