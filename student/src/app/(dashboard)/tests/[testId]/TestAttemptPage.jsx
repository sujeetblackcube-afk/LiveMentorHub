...
export default function TestAttemptPage() {
  ...
  const fetchTest = async () => {
    try {
      ...
      const foundTest = testsArray.find(
        (t) => String(t.submissionId) === String(submissionId),
      );
      if (!foundTest) {
  ...
  const getAnswerTextForDisplay = (questionId) => {
    const question = questions.find((q) => q.id === questionId);
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center justify-between mt-5 lg:mt-8">
              ...
              <Button
                ...
                disabled={currentQuestionIndex === questions.length - 1}
                className="px-6 lg:px-10 py-3 bg-gradient-[#0d1f5c] bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-base shadow-lg"
              >
                <span className="hidden lg:inline">Next</span>
                <span className="lg:hidden">Next</span>
                <ChevronRight className="h-5 w-5 lg:ml-2" />
