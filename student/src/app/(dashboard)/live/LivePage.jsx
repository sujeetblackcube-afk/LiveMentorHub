...
import StudentLiveVideo from "@/components/layout/StudentLiveVideo";
import {
  Video,
  Clock,
  Users,
...
export default function LivePage() {
  ...
  const [errorOngoing, setErrorOngoing] = useState(null);
  ...
  const fetchLiveSessionsByStatus = async (status) => {
  ...
  const getThumbnailGradient = (courseName) => {
  ...
  const getInstructorInitials = (name) => name.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2);
