...
const Avatar = ({ name, image }) => {
  ...
  return fullImageUrl ? (
    <img src={fullImageUrl} alt={name} className="w-9 h-9 rounded-full object-cover" />
  ) : (
...
export default function SuspendedStudents() {
  ...
  const downloadExcel = async () => {
    ...
    try {
      ...
      XLSX.writeFile(wb, "suspended_students.xlsx");
    } catch (err) {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      doc.text("Suspended Students List", 14, 25);
      ...
      doc.save("suspended_students.pdf");
    } catch (err) {
