...
// Hardcoded dummy courses for local dev (no network)
const DUMMY_COURSES = {
  'MATH-101': {
    courseCode: 'MATH-101',
    courseName: 'Algebra Foundations',
    courseDescription: 'Master the basics of algebra including linear equations, inequalities, and functions.',
    courseDuration: 30,
    difficulty: 'Beginner',
  },
  'SCI-201': {
    ...
    courseDescription: "Dive deep into kinematics, Newton's laws, and energy.",
    courseDuration: 60,
    difficulty: 'Advanced',
  },
  'ENG-105': {
...
export default function CourseInfo() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const found = DUMMY_COURSES[courseCode];
    // create a shallow copy so edits don't mutate the original constant
    setCourse(
      found
        ? { ...found }
        : {
  ...
  const handleSave = (e) => {
    ...
    setTimeout(() => {
      setSaving(false);
      // For now we just keep changes in memory — can persist to localStorage if desired
