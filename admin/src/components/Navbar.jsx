...
export default function Navbar({ collapsed, setCollapsed }) {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const response = await getNotificationBySuperAdminId();
      if (response.success) {
  ...
  const handleNotificationItemClick = (notification) => {
    ...
    if (type === "teacher") return navigate("/teacher/pending");
    if (type === "parent") return navigate("/parents");
    if (type === "course") return navigate("/course");
    if (type === "assignment") return navigate("/assignments");
    ...
    if (text.includes("teacher")) return navigate("/teacher/pending");
    if (text.includes("parent")) return navigate("/parents");
    if (text.includes("course")) return navigate("/course");
    if (text.includes("live") || text.includes("class")) return navigate("/managecontent");
    if (text.includes("test") || text.includes("exam")) return navigate("/managecontent");
    if (text.includes("assignment")) return navigate("/managecontent");
    if (text.includes("pay") || text.includes("subscrip")) return navigate("/subscription");
    if (text.includes("enroll")) return navigate("/enrollment");
  ...
  return (
    <header
      className={`fixed top-0 ${collapsed ? "left-16 sm:left-20" : "left-48 sm:left-64"} right-0 z-10 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 transition-all duration-300`}
      style={{
    ...
    >
      ...
      <div className="flex items-center gap-3 sm:gap-6 relative">
        ...
        <div className="relative" ref={notificationRef}>
          ...
          {notificationOpen && (
            <div
              className="absolute right-0 mt-2 w-72 sm:w-80 rounded-lg shadow-lg z-30 overflow-hidden"
              style={{
                backgroundColor: theme.colors.card,
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <div
              className="w-7 h-7 sm:w-9 sm:h-9 rounded-full text-white flex items-center justify-center font-semibold text-sm sm:text-base"
              style={{ backgroundColor: theme.colors.primary }}
            >
              {profileImage ? (
                <img
          ...
          {dropdownOpen && (
            <div
              className="absolute right-0 mt-2 w-40 sm:w-48 rounded-md shadow-lg z-20"
              style={{
                backgroundColor: theme.colors.card,
                border: `1px solid ${theme.colors.border}`,
              }}
