...
export default function SuspendedStudents() {
  ...
  const fetchSuspendedStudents = async () => {
    setLoading(true);
    try {
      const response = await getStudents({ status: "SUSPENDED" });
  ...
  const downloadExcel = async () => {
    ...
    if (dataToExport.length === 0) {
      throw new Error("No suspended students available to export");
    }

    const formattedData = dataToExport.map(mapStudentForExport);
    const ws = XLSX.utils.json_to_sheet(formattedData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Suspended Students");
    XLSX.writeFile(wb, "Suspended_Students_Report.xlsx");
  };
  ...
  const downloadPDF = async () => {
    ...
    doc.text("Suspended Students Report", 14, 15);
    ...
    doc.save("Suspended_Students_Report.pdf");
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="max-w-10xl mx-auto px-4 sm:px-6 py-4 flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-800">Suspended Students</h1>
      ...
      <div className="max-w-10xl mx-auto px-4 sm:px-6 py-4 flex flex-col lg:flex-row items-start lg:items-center gap-4 justify-between mb-6">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto">
          <FileDownloadZone
            ...
            fileName="Suspended_Students.xlsx"
            fileType="excel"
            onDownload={downloadExcel}
          />
          <FileDownloadZone
            title="Download PDF"
            fileName="Suspended_Students.pdf"
            fileType="pdf"
            ...
            <tbody className="divide-y text-center">
              {loading ? (
                ...
                paginatedStudents.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.studentId || idx} className={`hover:bg-gray-50 ${isSelected ? "bg-amber-50" : ""}`}>
